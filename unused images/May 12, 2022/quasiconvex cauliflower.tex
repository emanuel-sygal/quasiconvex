...
We first build certificates in the exterior unit disk $\mathbb D ^*$ and then transport them by the Böttcher coordinate $\psi$ of $f_{1/4}$ to the exterior of the cauliflower.
...
Our goal is to prove the quasiconvexity of $\mathrm{Exterior}(\mathcal{J})$, \cref{quasiconvex-cauliflower}. This parabolic case is more complicated than the hyperbolic case because the postcritical set $\PostCrit$ of $f$ accumulates at the parabolic fixed point $p=\frac 12$. Thus one no longer has a uniform bound on the distortion of inverse iterates, and we cannot immediately deduce quasiconvexity of the itinerary $\eta_{z_1,z_2}$ from the quasiconvexity of $\eta_{w_1,w_2}$ using Koebe's distortion theorem. We present an analogue of the principle of the conformal elevator in this parabolic setting.
...
\begin{definition}
	The \emph{reduced decomposition} of an itinerary $\eta$ is the unique decomposition $\eta=\gamma_1 + \delta_1 + \dots$ where each $\gamma_i$ is a concatenation of express tracks and followed by a single peripheral track $\delta_i$.
\end{definition}
...
\begin{proposition} \label{prop:finite-length}
	Let $z \in \mathcal J$, and let $\eta_z= \gamma_1 + \delta_1 + \dots  $ be the reduced decomposition of its itinerary. Then $\Length(\gamma_j)\lesssim \theta^j$ and $\Length(\delta_j)\lesssim \theta^j$ for some $\theta < 1$. In particular, $\Length(\eta_z)<\infty$ and all points $z\mathcal \in \mathcal J$ are accessible from the main station by a rectifiable curve.
\end{proposition}
...
\begin{lemma} \label{lemma-enough-visits-of-premain_station}
	Let $\gamma = \gamma_1 + \delta_1 + \dots $ be the reduced decomposition of an itinerary $\gamma$, and assume that $\gamma$ is not contained in the positive real axis. Then for every $k \geq 1$, there are $k$ iterates $n_1 < \dots < n_k$ such that $f^{\circ {n_i}}(\gamma_k) \subset \mathcal U_{-1}$. % This is actually an off-by-1 lie, since n_1=0 has no predecessor.
\end{lemma}
...
\begin{proof}[Proof (\cref{lemma-enough-visits-of-premain_station})]
	Every station $s \not \in [0,\infty)$ is a preimage of a station on the negative real axis. Thus every express reduced track $\gamma_i$ which is not contained in $[\tfrac 12, \infty)$ has a first iterate $n_i$ such that $f^{\circ n_i}(\gamma_i)$ is contained in the negative real axis, and in particular is contained in $\mathcal U_{-1}$. By definition of $\,\mathcal U_{-1}$, all tracks of the itinerary $f^{\circ n_i}(\gamma)$ that appear after $f^{\circ n_i}(\gamma_i)$ are contained in $\mathcal U_{-1}$ too, and in particular $f^{\circ n_i}(\gamma_k) \subset \mathcal U_{-1}$.
\end{proof}
\begin{proof}[Proof (\cref{prop:finite-length})] By the Schwarz lemma, any inverse branch $f^{-1}: \hat{\mathbb C} \setminus \Postcrit \to \hat{\mathbb C} \setminus \Postcrit$ is a contraction in the hyperbolic metric of the domain $\hat{\mathbb C} \setminus \Postcrit$. The contraction is strict as it is a composition of the contraction $\tilde f^{-1}:\hat{\mathbb C} \setminus \Postcrit \to \hat{\mathbb C} \setminus f^{-1} (\Postcrit)$ and the inclusion $\iota: \hat{\mathbb C} \setminus f^{-1} (\Postcrit)  \to \hat{\mathbb C} \setminus \Postcrit$ which is a strict contraction, as $f^{-1}(\PostCrit) \supset \mathcal \PostCrit$.

The domain $\mathcal U_{-1}$ is compactly contained in $\hat{\mathbb C} \setminus \Postcrit$, hence there is a bound $\Vert (f^{-1})' \Vert _{\mathrm{hyp}}< \theta < 1$ on $\mathcal U_{-1}$ that holds for both branches $f^{-1}: \mathcal U_{-1} \to \mathcal U_{\pm i}$. Then 
...
We conclude that $\Length(\gamma_k) \lesssim \theta^k$ for reduced tracks $\gamma_k$ that are disjoint from $\Delta$. In particular, this holds for all paths that start at the pre-main station $s_{1,1}$.
...
To estimate the length of express tracks, we introduce the notation
\begin{equation}
...
\begin{proof} \leavevmode
	...
	This property implies first that the sequence $a_k$ is Cauchy. Let $L$ be the limit. We have $L \leq 1$ since $\sum_{}\ell_k < \infty$. We cannot have $L<1$ since this would imply that $\ell_k \asymp \sum_{n=k}^\infty \ell_n$, contradicting part \emph{(i)}. Thus $\ell_k \asymp \ell _{k+1}$ as desired.
\end{proof}

\begin{definition}
The \emph{relative distance} of a curve $\gamma$ from the post-critical set is 
...
\begin{lemma}
There exists a constant $k>0$ such that for all $z_1,z_2 \in \mathcal J$, we have $|f(z_1)-f(z_2)|<k|z_1-z_2|$.
\end{lemma}

\begin{proof}
One may take $k=\lVert f|_{B}' \rVert _\infty$, where $B \subset \mathbb R^2$ is the union of all line segments having both endpoints on $\mathcal J$.  \textbf{Not exactly since $0 \in B$} makes the claim false.
\end{proof}
...
\begin{lemma}
For any two points $z_1,z_2$ on the Julia set of $f(z)=z^2+\frac 14$,
if $f(z_1)=f(z_2)$ then $|z_1-z_2|\geq c_0$ for a universal constant $c_0 > 0$.
\end{lemma}

\begin{proof}
Pulling back to the unit disk $\mathbb{D}$, the result holds for $z \mapsto z^2$.
\end{proof}

\subsection{Departure Sets}
...
We define constants $C_1,C_2, \epsilon$ as follows. We first choose $C_2 \ge 2$, then we let $C_1 = C_2 + d+2$ and choose $\epsilon >0$ small enough so that we have both
\begin{equation}
	\dist(\alpha_{C_1}, \alpha_{C_2}) \geq k \epsilon.
\end{equation}


We now show that the itineraries $\eta_{w_1,w_2}$ are certificates in three special cases:

%We will treat three special cases:
\begin{enumerate}
	\item $|w_1-w_2| \leq k \epsilon$, $\quad |m-n|<d$, $\quad m,n < C_1$; %relatively separated
	\item $|w_1-w_2| \leq k \epsilon$, $\quad |m-n|<d$, $\quad  m,n> C_2$; %relatively separated
...
For each $n$ we denote by  $\alpha_n$ the union of the two outermost tracks emanating from the station $s_{m,0}$. Namely, $\alpha_n$ has a positively-oriented peripheral track after each express track. We denote the domain enclosed by $\alpha_m, \alpha_n$ and $\mathcal J$ by $\mathcal K_{m,n}$, and denote the domain enclosed by $\mathcal J$ and $\alpha_n$ by $\mathcal K_n$.

% Cases \emph{1.} and \emph{2.} follow since they force the itinerary to be relatively separated from the post-critical set:

\begin{lemma}
Let $w_1,w_2 \in \mathcal J$ be points, with $w_1 \in I_m$ and $w_2 \in I_n$ for $m,n \geq 2$. Then the itinerary $\eta_{w_1,w_2}$ is contained in the domain $\mathcal K_{m-1,n+1}$.
\end{lemma}
...
Let $w_1,w_2 \in \mathcal J$. In each of the three special cases, the itinerary $\gamma _{w_1,w_2}$ is a quasiconvexity certificate. In cases 1 and 2, $\gamma _{w_1,w_2}$ is relatively separated.
 be as in case 1. Then the itinerary $\eta_{w_1,w_2}$ is relatively separated from the postcritical set.
\end{lemma}

\begin{proof}
\emph{case 2.}
Assume $n \geq m$, without loss of generality. Notice that the itinerary is contained in the domain $\mathcal K_{m-1,n+1}$, which has a positive relative distance from the cusp $p$.

\emph{case 1.} The itinerary in this case is contained in a domain of the form 
\begin{equation*}
\mathrm{Exterior}(\mathcal J) \cap B(0,R) \setminus \mathcal K_{C_1+1}. %$R=2L+|s_{0,0}|.
\end{equation*}
\end{proof}

In Case 3, the points $w_1,w_2$ lie in ``well-separated cusps''.
...
This gives some control from below on $|w_1-w_2|$. We now bound the length of the itinerary $\psi(\eta)=\psi(\eta_{w_1,w_2})$ from above.
...
We apply a stopping time argument to promote quasiconvexity of $\gamma_{w_1,w_2}$ to quasiconvexity of $\gamma{z_1,z_2}$, thereby proving the following:
...
Let $d\gg 1$ be a sufficiently large integer and let $(z_1,z_2)$ be a pair of points in $\mathcal J$. Repeatedly apply $f$ on $z_1,z_2$ until one of the following two stopping conditions occurs:
\begin{enumerate}[label=\normalfont(\roman*)]
...
In cases 1 and 2, we are done by \cref{case 1 rel. sep}. We therefore focus on case 3. We show that in case 3, the curve $f^{\circ (N-1)}(\gamma)$ is relatively separated.
