...
This also determines how to travel from the central station to a terminal $z\in \partial \mathbb D^*$, by continuity. See figure \ref{fig:Carleson1} and the next definition.
...
\begin{definition}
Let $z=\exp(2\pi i\theta)\in\partial\D$. The \emph{itinerary} $\eta_z$ from the central station to $z$ is a sequence of stations $\left(\sigma_{0},\sigma_{1},\ldots\right)$ where $\sigma_{0}=s_{0,0}=2$ and $\lim_{k\to \infty} \sigma_{k}=z$, defined as follows.

We choose the stations $\sigma_{i}$ inductively in pairs, in a greedy manner. In each step
we travel peripherally to the station closest to $z_{1}$ and then
...
We call $\eta_{z}$ the \emph{central itinerary} of $z$.
Note that there are no two consecutive peripheral tracks in $\eta_{z}$, and that the itineraries are invariant under $f_{0}$, in the sense that $$f_{0}(\eta_{z})=\eta{}_{f_{0}(z)}\cup[s_{0,0},f_0(s_{0,0})]$$
for every $z\in\partial\D$.

\begin{lemma}
Let $z\in \partial \mathbb D^*$. Decompose the central itinerary $\eta_{z}$ into its constituent tracks, 
$$\eta_{z}=\gamma _1 + \gamma_2\ldots .$$ 
	Then we have the estimate $$\mathrm{Length}(\gamma_{k})\lesssim2^{-k}$$ uniformly in $z$. 
...
Thus the total length of the itinerary is bounded uniformly in $z$.
\end{proof}

%In particular, the length of a suffix $\sigma_{k}+\sigma_{k+1}+\ldots$ decays exponentially in $k$, uniformly in $z$.
...
By continuity, this defines the itinerary between two terminals $z_1$ and $z_2$.

In a more precise language:
\begin{definition}
	Let $z_{1},z_{2}\in\partial\D^*$ be two terminals. The itinerary $\eta_{z_{1},z_{2}}$ between them is defined as follows.
	...
	and $\eta_{z_{2}}^{\text{truncated}}$ are two itineraries with a common
...
The domain $\D^{*}$ is quasiconvex with the itineraries $\eta_{z_1,z_2}$ as certificates.
\end{theorem}

\begin{proof}
It is enough to show that $\mathrm{Length}\left(\eta_{z_{1},z_{2}}\right)\lesssim\left|z_{1}-z_{2}\right|$.

As $\left|z_{1}-z_{2}\right|\asymp\left|\theta_{1}-\theta_{2}\right|$
and $\mathrm{Arg}\left(z_{i}\right)\propto\theta_{i}$, it is equivalent to show
\[
...
b.
\marginpar{hopefully} Since we already know that the lengths of tracks in the itinerary decay exponentially with rate $\theta>1$, the same proof of the case $c=0$ also shows quasiconvexity in this case.
...
Let $z_1,z_2 \in \mathcal J$, and let $\mathrm{CriticalOrbit(f)}=\{f^n(0): n \in \mathbb N\}$ be the forward orbit of the critical point.
As long as the distance $z_1-z_2$ is small in comparison to the distance of $z_1,z_2$ from $\mathrm{CriticalOrbit(f)}$, we profit from applying $f$: By standard distortion estimates, $|f(z_1)-f(z_2)|\geq c |z_1-z_2|$ for some constant $c>1$.
...
\bibitem{BLM16}Mario Bonk, Mikhail Lyubich, Sergei Merenkov, Quasisymmetries
