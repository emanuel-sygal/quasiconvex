...
See {[}2{]} for a stronger version, details and a proof.
...
We are now ready to show the analogue of theorem \ref{quasiconvex disk} for a hyperbolic map. \marginpar{TODO: We use more than hyperbolicity, also that the Julia set is a Jordan domain}
\begin{theorem} 
...
\marginpar{TODO: Make this correct }

%Since every journey is eventually
...
b. \marginpar{TODO: delete?} Let $z_{1},z_{2}$ be two points on $\mathcal{J}(f)$. We construct
...
We give a second proof, relying on the previous claim on the separation
...
the sum of the lengths $\mathrm{Length}\left(\eta_{z_{1}}\right)+\mathrm{Length}\left(\eta_{z_{2}}\right)\leq A\left|z_{1}-z_{2}\right|$
\end{comment}
\begin{comment}
