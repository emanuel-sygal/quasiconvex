...
the Cauliflower is a so-called \emph{John domain}, meaning intuitively that it has no outward-pointing cusps. See \cref{fig:julia_sets}.
...
\section{Complex Analytic Tools}
...
\begin{theorem}\cite[Theorem 2.9]{mcmullen_1994} \label{Koebe}
	Let $D \subset U $ be topological disks with modulus $\mathrm {Mod}(D, U) >m>0$ and let $f$ be a map univalent in $U$,
	then we have the bound 
	\begin{equation}
		\frac {|f(y)-f(z)|}{|y-z|} \asymp |f'(x)|
	\end{equation}
	for all $x,y,z \in D$.
\end{theorem}
...
Throughout this section we take $c$ in the main cardioid $\heartsuit$.
...
\end{equation*}
 be the (closed) post-critical set of $f$, and suppose that $f$ is not conjugate to $f_0: z\mapsto z^2$.
Then we have 
\begin{equation}
...
for every $z\in f^{-1}(\hat {\mathbb C} \setminus \PostCrit)$, and in particular for every $z \in \mathcal J$.
 ...
 is the hyperbolic \enquote{change of scale} of $f$ at the point $z$,
  where $\rho$ is the 
 hyperbolic (Poincaré) metric of the domain $\hat {\mathbb C} \setminus \PostCrit$.
...
\begin{proof}
	...
	By iterating $g$, the claim follows for all sufficiently small values of $\epsilon$.
\end{proof}
