...
\begin{doublespace}
\begin{center}
{\LARGE{}The escaping set of $z^2+1/4$ is quasiconvex}{\LARGE\par}
\par\end{center}
\end{doublespace}
...
$$
Its boundary $\mathcal J_c = \partial \mathcal K(f_c)$ is known as the {\em Julia set}\/, and its complement $\Exterior(\mathcal{J}_c) = \mathbb{C} \setminus \mathcal K(f_c)$ forms the {\em attracting basin of infinity}, also called the {\em escaping set}. The sets $\mathcal J_c$ and $\mathcal K_c$ are compact and are both forward and backward invariant under the dynamics of $f_c$.
...
A set $\Omega\subseteq\C$ is called \textbf{quasiconvex} 
...
in $W^{1,1}(\mathbb{C})$.
In \cite[Equation (1.1)]{strong_bv_extension_2022}, 
it is proved that a bounded Jordan domain 
is a $W^{1,1}$ extension domain if and only if its complement is quasiconvex.
% it is proved that a bounded simply connected domain $\Omega$
% is a $W^{1,1}$ extension domain if and only if its complement is quasiconvex via certificates that only touch the boundary $\partial \Omega$ at a set with zero Hausdorff measure $\mathcal H^1$.
...
An induction argument shows that $a_n \asymp 1/n$, and consequently, $\ell_n = a_n - a_{n+1} = a_{n+1}^2 \asymp 1/n^2$.
\end{proof}
\begin{comment}
