...
% Parameters $c \in \heartsuit$ are 
% {\em hyperbolic}. Loosely speaking, this means that $\mathcal J_c$ satisfies the principle of the conformal elevator: 
% up to bounded distortion, a small piece of $\mathcal J_c$ looks like a large piece $\mathcal J_c$.

% $\mathcal J_c$ is a \emph{quasicircle}, which roughly means that $\mathcal J_c$ has no \enquote{cusps}.
...
% \subsection{The Böttcher coordinate}
...
\begin{theorem}
	...
	There exists a Riemannian metric on the unit disk $\mathbb D$ which is invariant 
	under conformal automorphisms, and it is unique up to  multiplication by a positive constant.
\end{theorem}

\begin{theorem}
	Let $U$ be a planar domain whose complement $\mathbb C \setminus U$ has at least three points.
	Then the universal covering $\tilde U$ of $U$ is biholomorphic to $\mathbb D$. This defines the 
	Poincaré metric on $U$ as the unique Riemannian metric for which the projection 
...
$$
The map $\psi$ extends to a homeomorphism between the circle $\partial\D$ and $\mathcal{J}_c$ by Carathéodory's
theorem. Uniqueness is obvious from the uniqueness of the Riemann map up to rotation; 
see \cite[Theorem 9.5]{milnor_book} for a proof of existence, 
...
A quadratic map $f_c$ is called \emph{hyperbolic} if
...
Suppose $f_c: z\mapsto z^2+c$ is hyperbolic and $c \neq0$. 
...
%  Here 
%  \begin{equation}
% 	\lVert g'(z) \rVert_\mathrm{hyp} := \frac {\rho (g(z))}{\rho(z)} |g'(z)|,	
%  \end{equation}
%  is the hyperbolic \enquote{change of scale} of $f$ at the point $z$,
%   where $\rho$ is the 
...
\end{equation}
which follows from a distortion argument,
since $f^{\circ n}$ sends $(z_1,z_2)$ to $(w_1,w_2)$ and sends $\eta_{z_{1},z_{2}}$ to $\eta_{w_1,w_2}$.

Indeed, as $z_1$ and $z_2$ are $\epsilon$-close, 
we may find a topological ball $B$ containing the points $z_1,z_2$ 
and the itinerary $\eta_{z_1,z_2}$
such that $B$ has a definite modulus inside of $\hat{\mathbb C }\setminus \PostCrit$, 
and on which the map $f^{\circ n}$ is univalent.

We apply \cref{Koebe}
to deduce that 
\begin{equation}
	\frac {|w_1-w_2|}{|z_1-z_2|} \asymp |(f^{\circ n})'(x)|	
\end{equation}

for any point $x \in B$. On the other hand, applying $f^{\circ n}$ on the itinerary $\eta_{z_1,z_2}$ 
makes its length grow by the same factor, and we conclude that 
\begin{equation}
	\frac{|w_1-w_2|}{|z_1-z_2|}
	\asymp
	\frac{\Length\left(\eta_{w_1,w_2}\right)}{\Length\left(\eta_{z_{1},z_{2}}\right)},
\end{equation}
as needed.
\end{proof}
