...
\begin{proposition}
\label {case-3-proof}
There exists a sufficiently large integer $d$ so that whenever $m-n \geq d$, we have
	\begin{equation}
...
\begin{equation} \label{eq:gamma_m_not_large}
	...
	\end{equation}
for some constant $C \geq 0$.  \textcolor{red}{Maybe there is no branch of $f^{\circ -(m-n)}$ defined in a neighborhood of both?} Notice that (\ref{eq:gamma_m_not_large}) holds for $m=1$ by \cref{prop:finite-length}, which gives a uniform bound on the length of an itinerary.
