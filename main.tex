...
We deduce \cref{eq:hyperbolic-distortion-goal} from a distortion argument.
% since $f^{\circ n}$ sends $(z_1,z_2)$ to $(w_1,w_2)$ and sends $\eta_{z_{1},z_{2}}$ to $\eta_{w_1,w_2}$.

Indeed, denote $g=f^{\circ n}$. We may find a topological ball $B$ containing the points $w_1,w_2$ 
and the itinerary $\eta_{w_1,w_2}$
but with a definite modulus inside $\hat{\mathbb C }\setminus \PostCrit(g)$.
% TODO: this contains at least two frustrating details for showing why it exists. 
% Also is $\hat{\mathbb C }\setminus \PostCrit(g)$ really the correct domain?

We apply \cref{Koebe} on a branch of $g^{-1}$ in $B$ sending $(w_1,w_2)$ to $(z_1,z_2)$.
It follows that
\begin{equation}
