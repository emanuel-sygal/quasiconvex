...
Notice that the hyperbolic metric is equivalent to the Euclidean metric on compact subsets of $\mathbb D$.

\begin{theorem}\label{thm:criterion-hyperbolic-domain}
	Let $U$ be a planar domain. If the complement $\mathbb C \setminus U$ has at least two points
	then the universal covering $\tilde U$ of $U$ is biholomorphic to $\mathbb D$.
\end{theorem}

\begin{definition}
	Let $U$ be a planar domain whose universal covering $\tilde U$ is biholomorphic to $\mathbb D$,
...
\begin{proof}	
	...
	\begin{equation}
		\norm {Df_z^{\circ m}(v)}_\mathrm{hyp} \geq \kappa^m \norm v_\mathrm{hyp}
	\end{equation} 
	...
	we may take $n$ large enough so that for $g=f^{\circ m}$ there is a uniform bound
...
\textcolor{red}{Such a branch indeed exists in all of $B$, since $n$ is small relative to the \enquote{depth} of the itinerary,
so that $f^{\circ n}$ is indeed injective on a preimage of $B$?}
% since in Böttcher coordinates
% $f^{\circ n}$ is automatically injective on sectors away from $0$
%  of external angle smaller than $\frac {2\pi} {2^n}$, and 
%  indeed we cannot have $n$ larger than the depth of the itinerary 
%  $\eta_{w_1,w_2}$ because we stop soonner than that.

% Such a branch may not exist in all of $B$ if $n$ is large, since in Böttcher coordinates
% $f^{\circ n}$ is not injective on sectors of external angle larger than $\frac {2\pi} {2^n}$.
% Thus we work locally, using the inverse defined near each point of $\overline B$. by compactness a 
% finite number of such branches covers all of $B$.
