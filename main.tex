...
$$
 is a connected component of the interior of $\mathcal M$, which consists of the parameters $c \in \mathbb{C}$ for which $f_c$ has an attracting fixed point.
When $c \in \heartsuit$, the Julia set $\mathcal J_c$ is quasidisk: the image of the unit disk under a quasiconformal map.
...
the Cauliflower is a so-called \emph{John domain}, meaning intuitively that it has no outward-pointing cusps.
...
Explicitly, this means that there exists a constant $A\geq1$ such that every two points $z_{1},z_{2}\in\Omega$ are connected by a rectifiable path $\gamma:\left[0,1\right]\to\Omega$ which satisfies
$$
\Length(\gamma)\leq A\cdot\left|z_{1}-z_{2}\right|.
$$
We call such a path $\gamma$ a \emph{quasiconvexity certificate} for the pair $(z_{1}, z_{2})$.
...
$$ 
These are exactly the iterated preimages of the \emph{central station} $s_{0,0} = 2$ under the map $f_{0}$.
...
$\frac{\Length\left(\eta_{w_1,w_2}\right)}{|w_1-w_2|}$.

Since $f^{\circ n}$ sends $(z_1,z_2)$ to $(w_1,w_2)$ and $\eta_{z_{1},z_{2}}$ to $\eta_{w_1,w_2}$, 
a distortion argument gives that
\begin{equation}
	\frac{\Length\left(\eta_{z_{1},z_{2}}\right)}{|z_1-z_2|}\asymp\frac{\Length\left(\eta_{w_1,w_2}\right)}{|w_1-w_2|}.
\end{equation}
and we are done. Indeed, since $z_1$ and $z_2$ are $\epsilon$-close, 
we may find a topological ball $B$ containing the points $z_1,z_2$ 
and the itinerary $\eta_{z_1,z_2}$
with a definite modulus inside of $\hat{\mathbb C }\setminus \PostCrit$, 
on which the map $f^{\circ n}$ is univalent, 
and apply Koebe's distortion theorem in the version of \cite[Theorem 2.9]{mcmullen_1994} 
to deduce that $\frac {|w_1-w_2|}{z_1-z_2} \asymp |(f^{\circ n})'(x)|$ 
for any point $x \in B$, but applying $f^{\circ n}$ on the itinerary $\Length\left(\eta_{z_1,z_2}\right)$ 
makes its length grow by the same factor.
\end{proof}
