...
To prove \cref{quasiconvex-cauliflower} we develop a version of the conformal elevator principle for parabolic maps. Given a pair of points $z_1,z_2$, by iteratively applying $f$ we can make them a definite distance apart, since the map is expansive on its Julia set. However, we can't always apply Koebe's distortion theorem to deduce an estimate on the path between the original points $z_1,z_2$. The problem arises when $z_1,z_2$ get mapped to points too close to the cusp $\frac 12$, so that the inverse map is not univalent. Thus we have a second stopping condition when we iterate $f$ on the pair of points: we stop if they lie in sufficiently well-separated cusps. 

There are two cases: If initially the points $z_1,z_2$ are in the same cusp: ?

WE fix a small ball $B$ around $1/2$

We decompose the points of the circle according to the first \emph{departure}\,: the first time that the central itinerary made a turn.
