...
\end{lemma}
\begin{proof} \leavevmode
	The map $f$ is affinely conjugate to the map $g: z\mapsto z^2+z$, and the fixed point $\frac 12$ of $f$ corresponds to the fixed point $0$ of $g$. A point near $0$ get attracted to $0$ at rate $\asymp 1n$, so the distance between consecutive images in the trajectory is $\asymp \frac 1{k^2}$. Hence $\ell_k \asymp \frac 1{k^2}$.
\end{proof}
