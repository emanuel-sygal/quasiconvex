...
A quadratic map $f_c$ is (dynamically) \emph{hyperbolic} if
 ...
 \begin{theorem}
Let $f_c$ be a hyperbolic quadratic map, and let $\lVert \cdot \rVert_\mathrm{hyp}$ be the 
...
\begin{corollary} \label{elevator for points on julia}
	...
	There exists  $\epsilon > 0$ such that every pair of points $z,w\in\mathcal{J}(f)$ has a forward iterate $f^{\circ n}$ for which $\left|f^{\circ n}(z)-f^{\circ n}(w)\right|>\epsilon$.	
\end{corollary}
