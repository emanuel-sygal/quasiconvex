...
The following corollary says that nearby points are connected by a short itinerary.
\begin{corollary}
For every $\epsilon>0$ there exists $\delta$ such that for any pair points $(w_1,w_2)$ with distance $|w_1-w_2|<\delta$, we have $\Length(\eta_{w_1,w_2}) < \epsilon$.
\end{corollary}
...
\begin{proof} \leavevmode
		...
		This property implies first that the sequence $(a_k)$ is bounded and then that it is Cauchy. Let $L$ be its limit, then we cannot have $L>1$ since $\sum_{}\ell_k < \infty$. On the other hand we cannot have $L<1$ either since this would imply that $\ell_k \asymp \sum_{n=k}^\infty \ell_n$, contradicting part \emph{(i)}. Thus $\ell_k \asymp \ell _{k+1}$ as desired.
\end{proof}
