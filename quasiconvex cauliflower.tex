...
This also determines how to travel from the central station to a terminal $\zeta\in \partial \mathbb D^*$, by continuity. See \autoref{fig:Carleson1} and the next definition.
...
The itinerary between two stations $s$ and $s'$ is defined by juxtaposing $\eta_{s_1}$ and $\eta_{s_2}$ and discarding the part where they coincide. See \autoref{fig:Carleson2}. 
...
\begin{definition} \label{def-disk-itinerary}
	The \emph{itinerary} between two terminals $\zeta_{1},\zeta_{2}\in\partial\D^*$ is defined by gluing the central itineraries $\eta_{\zeta_{1}},\eta_{\zeta_{2}}$ and discarding the common part. Explicitly, 
	denote $\eta_{\zeta_{1}}=\left(\sigma_{n}^{1}\right)_{n=0}^{\infty},\eta_{\zeta_{2}}=\left(\sigma_{n}^{2}\right)_{n=0}^{\infty}$
	and let $\left(\sigma_{0},\ldots,\sigma_{N}\right)$ be the common prefix of $\eta_{\zeta_{1}}$ and $\eta_{\zeta_{2}}$. Let $\eta_{\zeta_{i}}^{\text{truncated}}=\left(\sigma_{N},\sigma_{N+1}^{i},\ldots\right)$
	be the truncated paths. Then
	%= \operatorname{Reverse}()...	
 \begin{gather*}
 \eta_{\zeta_{1},\zeta_{2}}  \left(\dots,\sigma_{N+2}^{2},\sigma_{N+1}^{2},\sigma_{N},\sigma_{N+1}^{1},\sigma_{N+2}^{1},\dots\right).
 \end{gather*}
	This is a simple bi-infinite path connecting $\zeta_{1}$ and $\zeta_{2}$.
...
\begin{proof}
We keep the notations of \autoref{def-disk-itinerary}.
Decompose 	
\begin{equation}
\Length(\eta_{\zeta_{1},\zeta_{2}})=\Length\left(\eta_{\zeta_{1}}^{\text{truncated}}\right)+\Length\left(\eta_{\zeta_{2}}^{\text{truncated}}\right),
\end{equation}
then each summand is bounded using \autoref{track_decay} and \eqref{generation-lower-bound} by 
\[
...
\]
since the central itineraries to $\zeta_1$ and to $\zeta_2$ diverged on $\sigma_N$, which lies on generation $\geq \frac N2$ since there are no consecutive peripheral tracks.
...
\begin{comment}
\begin{proof}
	Apply \autoref{elevator} to a ball centered on the Julia set which contains $z,w$ on its boundary at roughly antipodal points. After blowing up we get points $f^{\circ n}(z),f^{\circ n}(w)$ which are a definite distance apart by Koebe's distortion theorem. %\todo{make this correct}
\end{proof}
\end{comment}

We are now ready to show the analogue of \autoref{quasiconvex disk}.
...
We give a second proof, relying on \autoref{elevator for points on julia}. This proof will better prepare us for the
parabolic $c=1/4$ case, in which we don't have expansion of $f$ on its Julia set.

By \autoref{elevator for points on julia}, there exists an $\epsilon>0$ such that any two points are $\epsilon$-apart under some iterate $f$. 
...
we use  \autoref{elevator for points on julia} to find an iterate $f^{\circ n}$ such that 
\begin{equation}
...
To prove \autoref{quasiconvex-cauliflower} we develop some machinery. We decompose the points of the circle according to 
...
See \autoref{fig:departure-decomposition}.
\end{definition}
...
\begin{theorem}[Parabolic Conformal Elevator on $\mathcal J$] \label{parabolic-elevator}
	...
	\end{enumerate}
\end{theorem}
 \todo{Mostly unwritten from here}
...
\autoref{parabolic-elevator} reduces \autoref{quasiconvex-cauliflower} to the case when $f^{\circ N }(z_1) \in \psi(I_n)$ and $f^{\circ N}(z_2) \in \psi(I_m)$ with $|m-n| \geq d$.
...
Condition \eqref{parabolic separation} gives us some control from below on $|z_1-z_2|$. We compare it to the length of the itinerary $\psi(\eta)=\psi(\eta_{z_1,z_2})$ to finish the proof.

 \input{"departure-decomposition.tex"}

We represent $\eta$ as a concatenation of three paths: The radial segment $[s_{m,0},s_{n,0}]$ and the two other components, $\gamma _m$ and $\gamma_n$. See \autoref{fig:Three-parts-of-eta}.
...
% This concludes the proof of \autoref{quasiconvex-cauliflower}.
