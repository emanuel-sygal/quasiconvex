...
We first need to know that each itinerary $\eta_{z_1,z_2}$ has a finite length. This is a corollary of the following:
\begin{proposition}
	For every $z \in \mathcal J$ we have $\Length(\eta_z)<\infty.$
\end{proposition}

\begin{proof}
Decompose $\eta_z$ to its constituent tracks. The lengths of the peripheral tracks decays exponentially, because any two peripheral tracks are separated by at least one express track.
\end{proof}
...
To estimate the length of each summand, we introduce the line segments $\ell_k = \psi(s_{k,0})-\psi(s_{k+1,0}).$ 
...
\begin{lemma} \label{lem-ell_n}
	...
	\begin{align*}
		\Length(\psi(\gamma_{m,n})) = \ell_{m}+\ldots+\ell_{n} \\ \leq C (\ell_m+\ell_n)
	\end{align*}
	whenever $|m-n| \geq d$.
\end{lemma}
\begin{proof} \leavevmode
	The map $f$ is affinely conjugate to the map $z^2+z$, and the main cusp $\frac 12$ corresponds to $0$, hence $\ell_n \asymp \frac 1n$.
\end{proof}
...
\begin{align} \label{eq:triang}
	\begin{split}
	\Length(\psi(\gamma_{m,n})) \leq |w_1-w_2|+\Length(\psi(\gamma_m))+\Length(\psi(\gamma_n)) %\nonumber
...
\begin{align*}
\Length(\psi(\gamma_{m,n})) \lesssim \Length(\psi(\gamma_{m,n})) - C(\ell_m + \ell _n)
\end{align*}
