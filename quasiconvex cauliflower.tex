...
In this section $c=\frac 14$, so $f=f_{1/4}: z\mapsto z^2+ \frac 14$.
...
Unlike the hyperbolic case, the postcritical set of $f$ has an accumulation point $p=\frac 12$ on $\mathcal J$. If the distance between iterates $w_1=f^{\circ n}(z_1)$ and $w_2=f^{\circ n}(z_2)$ is too big in comparison to the distance between $w_1$ and $p$, then the distortion estimate on $f^{\circ (-n)}(\eta_{w_1,w_2})$ degenerates and we cannot conclude quasiconvexity of $\eta_{z_1,z_2}$ from quasiconvexity of $\eta_{w_1,w_2}$. 

To prove part (ii) of \cref{quasiconvex-cauliflower} we thus consider separately the case where $w_1$ is closer to the main cusp $p=\frac 12$ than to $w_2$. In this case we use moduli estimates to show quasiconvexity directly. To formulate this second case, we decompose the points of the circle according to the first \emph{departure}\,: the first time that the central itinerary made a turn.
...
The following theorem shows that to check quasiconvexity of a general pair of points $z_1,z_2$ it is enough to consider the case where $z_1,z_2$ lie in well-separated cusps near the main cusps.

\begin{theorem}[Parabolic Conformal Elevator on $\mathcal J$] \label{parabolic-elevator}
	Let $d\geq 2$ be an integer and let $z_1,z_2 \in \mathcal J$ be a pair of points. Repeatedly apply $f$ on $z_1,z_2$ until one of the following two stopping conditions happens:
	\begin{enumerate}[label=\normalfont(\roman*)]
...
Denote $f^{\circ N}(z_i)$ by $w_i$. Suppose that the itinerary $\eta_{w_1,w_2}$ satisfies
\begin{equation*}
	\Length(\eta_{w_1,w_2}) \leq A |w_1-w_2|
\end{equation*}
for some $A>0$. Then the original points $z_1,z_2$ enjoy a similar estimate,
\begin{equation*}
...
%\cref{parabolic-elevator} reduces \cref{quasiconvex-cauliflower} to the case when $f^{\circ N }(z_1) \inI_n$ and $f^{\circ N}(z_2) \in I_m$ with $|m-n| \geq d$.

By \cref{parabolic-elevator}, it is enough to show that the itinerary $\eta_{z_1,z_2}$ is a certificate for points that satisfy
\begin{align} \label{parabolic separation}
	z_1 \in I_n, \quad z_2 \in I_m, \quad m-n \geq d.
\end{align}
